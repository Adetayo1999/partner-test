...
export const Dropdown: React.FC<InputProps> = React.forwardRef(
  ({ labelText, className, error, options, ...rest }, ref) => {
    const renderInputLabel = () => {
      if (typeof labelText === "string")
        return (
          <label className={`text-xs font-semibold ${error && "text-red-500"}`}>
            {labelText}
          </label>
        );
      return labelText;
